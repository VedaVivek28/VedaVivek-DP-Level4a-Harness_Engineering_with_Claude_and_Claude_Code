...
- Model(s): claude-haiky-4-5-20251001
...
8. **Path-scoped rules.** Quote the glob frontmatter from one rule file. Why is it better than a directory-level CLAUDE.md for cross-cutting conventions?
   → In .claude/rules/react.md, the frontmatter declares: globs: ["src/components/**/*.tsx", "src/pages/**/*.tsx"]
...
13. **Small state.** Byte size of your `hot_state.json`. Why does the budget matter for a system run once per shift, indefinitely?
    → ls -l data/hot_state.json yielded 1,248 bytes. Keeping hot state strictly budgeted is important because the system runs once per shift indefinitely; unconstrained state accumulation would cause memory leak, slowing down serialization, and followed by exceed context boundaries when reloaded across shifts.
...
16. **Context, two faces.** Compare context management in System 2 (intra-session) and System 4 (cross-session) with cited numbers from both. Same principle, different mechanism — how?
    → Both systems enforce context minimization to prevent degradation, even though they apply different architectural mechanisms. System 2 manages intra-session context dynamically in-memory, shrinking 38,708 transcript tokens down to 16,851 tokens via persistent case facts and hierarchical summarization. System 4 manages cross-session context externally by pushing history entirely out of the prompt into tiered storage, querying only 8 defects via SQL, and capping hot state to 1,248 bytes.
